...
function resolveWebshellAiStreamRequest() {
    ...
    return apiFetch('/api/config').then(function (r) {
        ...
        if (typeof window.csaiChatAgentMode === 'object' && typeof window.csaiChatAgentMode.normalizeStored === 'function') {
            norm = window.csaiChatAgentMode.normalizeStored(localStorage.getItem('MeowCyber-chat-agent-mode'), cfg);
        } else {
            var mode = localStorage.getItem('MeowCyber-chat-agent-mode');
...
function wsInitAgentMode() {
    ...
    apiFetch('/api/config').then(function (r) { return r.ok ? r.json() : null; }).then(function (cfg) {
        ...
        // 标准化当前值
        var stored = localStorage.getItem('MeowCyber-chat-agent-mode');
        var norm;
        if (typeof window.csaiChatAgentMode === 'object' && typeof window.csaiChatAgentMode.normalizeStored === 'function') {
...
function wsSelectAgentMode(mode) {
    try { localStorage.setItem('MeowCyber-chat-agent-mode', mode); } catch (e) { /* */ }
...
function wsRefreshSelectors() {
    wsUpdateRoleSelectorDisplay();
    wsRenderRoleList();
    var stored = localStorage.getItem('MeowCyber-chat-agent-mode') || 'eino_single';
